...
public class LibrarianCommandLine {
    public static void main(String[] args) {
        ...
        // Diplay the menu.
        displayOptions();
        // Declare a String variable to hold the value of user input.
        String librarianInput;
        do {
            librarianInput = getLibrarianOption(scanner);
            // Check the different possible options.
            if (librarianInput.equals("1")) {
                the_guy.findMeAvailableBooks();
            } else if (librarianInput.equals("2")) {
                the_guy.findMeBook(getBookTitle(scanner));
            } else if (librarianInput.equals("3")) {
                the_guy.findMeBooksFromAuthor(getAuthorName(scanner));
            }
            // Diplay the menu if the user does not exit.
            if (!librarianInput.equals("q")) displayOptions();
        } while (!librarianInput.equals("q"));
        // Close the scanner.
        scanner.close();
    }

    /**
     * Displays the options to the user - librarian the guy.
     */
    ...
    /**
     * Prompt user to select from the options above
     * and returns his option.
     */
    private static String getLibrarianOption(Scanner scanner) {
        String input;
        do {
            System.out.print("> ");
    ...
    private static boolean isValidInput(String input) {
        // Check if the input is longer than one letter.
        if (input.length() != 1) return false;
        // Check if input is between 1 to 3 and q.
        if (!input.matches("[1-3q]+")) return false;
        return true;
    }
}