...
public class StudentDAO {
	...
	private static final String STUDENT_RM_TAG = "rm";
	...
	public void registerStudent(Student student) throws Exception {
		String sql = "INSERT INTO " + STUDENT_TABLE + "(" + STUDENT_RM_TAG + ", " + STUDENT_NAME_TAG + ", "
		...
		try {
			...
			preStmt.setString(1, student.getRm());
			...
			throw new SQLException("Student with RM: " + student.getRm() + " already exists!");
		} catch (Exception e) {
	...
	/**
	 * Finds the Student with the given rm
	 * 
	 * @param rm
	 * @return The Student with the specified rm
	 * @throws Exception
	 */
	public Student findStudentByAm(String rm) throws Exception {
		Student student = null;
		String sql = "SELECT * FROM " + STUDENT_TABLE + " WHERE " + STUDENT_RM_TAG + "=?;";
		DAOUtils.prepareResources(conn, preStmt, rs);
		try {
			conn = DAOUtils.getConnection();
			preStmt = conn.prepareStatement(sql);
			preStmt.setString(1, rm);
	...
	private static Student populate(ResultSet rs) throws SQLException {
		...
		student.setRm(rs.getString(STUDENT_RM_TAG));
